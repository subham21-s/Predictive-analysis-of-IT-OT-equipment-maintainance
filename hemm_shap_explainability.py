...
# ── Handle all SHAP versions ──────────────────────────────────
# Older SHAP : list of arrays [class0, class1], shape (n_samples, n_features)
# Newer SHAP : single 3D array, shape (n_samples, n_features, n_classes)
if isinstance(shap_values, list):
    sv_failure = shap_values[1]              # class 1 = Failure
    ...
    sv_failure = shap_values                 # single output model
    ev = explainer.expected_value
    base_val = float(ev[0]) if hasattr(ev, '__len__') else float(ev)

print(f"  SHAP values computed for {SHAP_SAMPLE} test samples")
print(f"  Shape: {sv_failure.shape}  (samples x features)")
print(f"  Base value (avg failure rate): {base_val:.4f}")
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  Base Rate           : {base_val*100:.1f}%  (average failure rate)
