...
# Auto-find any CSV inside the data/ folder
DATA_DIR = os.path.join(BASE_DIR, "data")
os.makedirs(DATA_DIR, exist_ok=True)

# Try common known filenames first, then pick any CSV found
CANDIDATE_NAMES = [
...
# If still not found, pick any CSV in data/ folder
...
if DATA_PATH is None or not os.path.exists(DATA_PATH):
    ...
    raise FileNotFoundError(
        f"\n❌ No CSV dataset found in: {DATA_DIR}"
        f"\n   Files currently in data folder: {csv_files if csv_files else 'EMPTY'}"
        f"\n\n   ✅ FIX: Copy your CSV file into the data folder:"
        f"\n   {DATA_DIR}\\"
        f"\n\n   Your CSV files in the project root that need to be moved:"
        f"\n   - HEMM_Dataset_FINAL.csv"
        f"\n   - HEMM_Dataset_ENHANCED.csv"
    )

print(f"  Dataset found : {os.path.basename(DATA_PATH)}")
